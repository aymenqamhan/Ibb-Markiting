...
if ($stmt === false) {
    error_log("Failed to prepare approved orders query: " . $con->error);
    $message = "<div class='alert alert-danger'>حدث خطأ في قاعدة البيانات أثناء جلب الطلبات المعتمدة.</div>";
} else {
    $stmt->execute();
...
<head>
    ...
    <title>إدارة الطلبات - الطلبات المعتمدة</title>
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <h2 class="mb-4 text-success"><i class="bi bi-check-circle-fill me-2"></i> الطلبات المعتمدة</h2>
            <?php echo $message; ?>

            <?php if (empty($orders)): ?>
                <div class="alert alert-info text-center" role="alert">
                    <i class="bi bi-info-circle me-2"></i> لا توجد طلبات معتمدة حالياً.
                </div>
            <?php else: ?>
                            ...
                            <?php foreach ($orders as $order): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="process_order.php?order_id=<?php echo $order['id']; ?>&action=shipped" class="btn btn-primary btn-sm" title="تم الشحن"><i class="bi bi-truck-flatbed"></i></a>
                                        <a href="process_order.php?order_id=<?php echo $order['id']; ?>&action=delivered" class="btn btn-success btn-sm" title="تم التوصيل"><i class="bi bi-check-all"></i></a>
